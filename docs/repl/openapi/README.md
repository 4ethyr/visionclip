...
A especificação usa `openapi: 3.1.0` por compatibilidade ampla com Swagger UI e geradores de cliente. O runtime Rust continua sendo a fonte de verdade; qualquer alteração nos enums `ReplCommand`, `ReplEvent`, `ReplSession` ou `JobResult` deve atualizar este arquivo.
